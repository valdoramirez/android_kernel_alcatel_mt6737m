...
//PR799863 added by chuanbo qiu for otp support begin
#include <linux/kernel.h>
#include <linux/syscalls.h>
#include <linux/fcntl.h>
#include <asm/segment.h>
#include <linux/buffer_head.h>
//PR799863 added by chuanbo qiu for otp support end
...
#define TRACE_REGION "/dev/block/platform/mtk-msdc.0/11230000.msdc0/by-name/proinfo" //"/dev/pro_info"
...
static void id_offset(struct otp_ctl *p) {
    ...
    if(fd >= 0) {
        ...
        if (len < TRACE_REGION_SZ) {
            printk("[EMMC_OTP]read %s file failed, len=%d\n", TRACE_REGION, len);
            goto fail;
        } else {
            printk("[EMMC_OTP]read %s ok, len=%d\n", TRACE_REGION, len);
        }
        sys_close(fd);
    } else {
        printk("[EMMC_OTP]can not open %s file\n", TRACE_REGION);
        goto fail;
    }
...
unsigned int emmc_otp_read(unsigned int blk_offset, void *BufferPtr)
{
	...
	//PR799863 added by chuanbo qiu for otp support
