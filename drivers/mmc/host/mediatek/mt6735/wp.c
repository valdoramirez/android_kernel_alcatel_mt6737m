...
{
	unsigned char l_ext_csd[512],wp_group_size;
...
static void emmc_wp_info(unsigned int end_sector)
{
    ...
    msdc_cmd.opcode = MMC_SEND_WRITE_PROT_TYPE;
...
static int wp_info_show_proc(struct seq_file *m, void *v)
{
	...
	disk_part_iter_init(&piter, disk, 0);
    region_wp_type = region_in_bitmap(0,1024); //for pgpt
    seq_printf(m, "%-16s 0x%016llx\t0x%016llx\t%-16s\n", "pgpt", 0ULL, 8 * 1024 * 1024ULL,type2str(region_wp_type));
    ...
    region_wp_type = region_in_bitmap(last,1024); //for sgpt
    seq_printf(m, "%-16s 0x%016llx\t0x%016llx\t%-16s\n", "sgpt", last, 512 * 1024ULL,type2str(region_wp_type));
