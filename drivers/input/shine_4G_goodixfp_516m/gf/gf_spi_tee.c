...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	case GF_IOC_INPUT_KEY_EVENT:
		...
		} else if (GF_KEY_MENU == gf_key.key) {
...
/* add by peng.tian.hz for fp info begin*/
static char vendor_id[2] = {1, 1};
static ssize_t fp_info_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char fp_dev_info[32] = "NULL:NULL:NULL:NULL";
	if (vendor_id[0] == 0) {
		sprintf(fp_dev_info, "%s:%s:%s:%s", "goodix", "GF516M", "SUP1", "NULL");
	}
	return sprintf(buf, "%s\n", fp_dev_info);
}

DEVICE_ATTR(FP, S_IRUGO, fp_info_show, NULL);
...
/* add by peng.tian.hz for fp info end*/
