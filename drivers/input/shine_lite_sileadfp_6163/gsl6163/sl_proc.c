...
static void sl_proc_cmd_uevent(char *cmd)
{
    ...
    if (fp_spidev){
       kobject_uevent_env(&fp_spidev->spi->dev.kobj, KOBJ_CHANGE, env_ext); 
    }
//       kobject_uevent_env(&fp_spidev.kobj, KOBJ_CHANGE, event); 
...
static void sl_proc_cmd_reg(char*cmd)
{
    ...
    if (addr &(~0xFF)){
        ret = spidev_write_reg(fp_spidev, (addr >>7), 0xF0);
	...
	printk("%s,%s,%d\n", __func__, fp_spidev->mmap_buf, ret);
