...
/*add by zhaixiaobin close ps and update function*/
...
void focaltech_get_upgrade_array(void)
{
	...
	if(chip_id==0)
	{
		if(tpd_vendor_id == CTP_VENDOR_LCE)
	{
               chip_id=0x64;
	}
...
/*Begin xiaopu.zhu for add 3nd ctp 20160113*/
	...
	if(uc_reg_value != 0x87 && uc_reg_value != 0xA1)
	{
	...
	if(uc_reg_value == 0x87)
	{
		tpd_vendor_id = CTP_VENDOR_LCE;
	}
	else
	{
		...
		tpd->tp_vendor_id = tpd_vendor_id;
...
void ft_resume(void)
 {
/*Begin xiaobin.zhai 20170207 for defect 4058055*/	
#ifdef TPD_PROXIMITY
	u8 state;
#endif
/*End xiaobin.zhai 20170207 for defect 4058055*/	
...
/*Begin xiaobin.zhai 20170207 for defect 4058055*/	
	...
	{
		i2c_smbus_read_i2c_block_data(i2c_client, 0xB0, 1, &state);
		TPD_DMESG("ft_resume [proxi_fts]read: 999 0xb0's value is 0x%02X\n", state);
		if (state == 0)
		{
			state |= 0x01;
			i2c_smbus_write_i2c_block_data(i2c_client, 0xB0, 1, &state);
			TPD_DMESG("ft_resume [proxi_fts]write: 0xB0's value is 0x%02X\n", state);
		}
	}
#endif //TPD_PROXIMITY
/*End xiaobin.zhai 20170207 for defect 4058055*/	
