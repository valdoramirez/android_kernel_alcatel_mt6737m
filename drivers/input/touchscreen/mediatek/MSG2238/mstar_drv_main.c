...
#ifdef CONFIG_ENABLE_PROXIMITY_ASSISTANCE_BY_GSENSOR
static struct proc_dir_entry *_gProcProxAssistanceEntry = NULL;
...
#ifdef CONFIG_ENABLE_PROXIMITY_ASSISTANCE_BY_GSENSOR
static const struct file_operations _gProcProxAssistance= {
    .read = DrvMainProcfsProxAssistanceRead,
    .write = DrvMainProcfsProxAssistanceWrite,
};
#endif //CONFIG_ENABLE_PROXIMITY_ASSISTANCE_BY_GSENSOR
...
#ifdef CONFIG_ENABLE_PROXIMITY_ASSISTANCE_BY_GSENSOR
u8 g_ProximityAssistanceMode = PROX_ASSIST_MODE_NORMAL;
#endif //CONFIG_ENABLE_PROXIMITY_ASSISTANCE_BY_GSENSOR
...
#endif //CONFIG_ENABLE_GLOVE_MODE
#ifdef CONFIG_ENABLE_PROXIMITY_ASSISTANCE_BY_GSENSOR
ssize_t DrvMainProcfsProxAssistanceRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    u32 nLength = 0;
    ...
    DBG("g_ProximityAssistanceMode = 0x%x\n", g_ProximityAssistanceMode);

    nLength = sprintf(pBuffer, "%x", g_ProximityAssistanceMode);
...
ssize_t DrvMainProcfsProxAssistanceWrite(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    u32 nProximityAssistanceMode = 0;
    u32 i = 0;
    char *pCh = NULL;
    ...
    if (pBuffer != NULL)
    {
        if (g_EnableTpProximity != 1) {
            DBG("Proximity function is not enabled\n");
            return nCount;
        }
        ...
        while ((pCh = strsep((char **)&pBuffer, ",")) && (i < 1))
        {
            ...
            nProximityAssistanceMode = DrvCommonConvertCharToHexDigit(pCh, strlen(pCh));

            i ++;
        }

        DBG("nProximityAssistanceMode = 0x%x\n", nProximityAssistanceMode);

		if (g_ProximityAssistanceMode == nProximityAssistanceMode) {
            DBG("Already in mode 0x%x, skip set up\n", nProximityAssistanceMode);
            return nCount;
        }
        else if (nProximityAssistanceMode >= PROX_ASSIST_MODE_UNKNOWN) {
            DBG("Unknown mode 0x%x, skip set up\n", nProximityAssistanceMode);
            return nCount;
        }

        switch (nProximityAssistanceMode) {

            case PROX_ASSIST_MODE_WEAK:
                // Mode weak
                DrvIcFwLyrSetProxAssistanceMode(PROX_ASSIST_MODE_WEAK);
                break;

            case PROX_ASSIST_MODE_NORMAL:
                // Mode normal
                DrvIcFwLyrSetProxAssistanceMode(PROX_ASSIST_MODE_NORMAL);
                break;

            case PROX_ASSIST_MODE_ENHANCED:
                // Mode enhanced
                DrvIcFwLyrSetProxAssistanceMode(PROX_ASSIST_MODE_ENHANCED);
                break;

            default:
                // we take default as normal
                DrvIcFwLyrSetProxAssistanceMode(PROX_ASSIST_MODE_NORMAL);
                break;
        }
    }
...
void DrvMainRemoveProcfsDirEntry(void)
{
    DBG("*** %s() ***\n", __func__);
#ifdef CONFIG_ENABLE_PROXIMITY_ASSISTANCE_BY_GSENSOR
    if (_gProcProxAssistanceEntry != NULL)
    {
        remove_proc_entry(PROC_NODE_PROXIMITY_ASSISTANCE, _gProcDeviceEntry);
        _gProcProxAssistanceEntry = NULL;
        DBG("Remove procfs file node(%s) OK!\n", PROC_NODE_PROXIMITY_ASSISTANCE);
    }
#endif //CONFIG_ENABLE_PROXIMITY_ASSISTANCE_BY_GSENSOR
#ifdef CONFIG_ENABLE_GLOVE_MODE
...
#ifdef CONFIG_ENABLE_PROXIMITY_ASSISTANCE_BY_GSENSOR
    _gProcProxAssistanceEntry = proc_create(PROC_NODE_PROXIMITY_ASSISTANCE, PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcProxAssistance);
    if (NULL == _gProcProxAssistanceEntry)
    {
        DBG("Failed to create procfs file node(%s)!\n", PROC_NODE_PROXIMITY_ASSISTANCE);
    }
    else
