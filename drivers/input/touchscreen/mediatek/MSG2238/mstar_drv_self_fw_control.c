...
{
//Modified by xiaobin.zhai for Task 4593652 on 2017/04/13 begin
        #include "Buzz6T_4G_EACH_MSG2238_V0x1.08_20170410.h"
//Modified by xiaobin.zhai for Task 4593652 on 2017/04/13 end
};
...
//add by xiaobin.zhai for hardwareinfo
...
//Modified by xiaobin.zhai for Task 4593652 on 2017/04/13 begin
    if (g_ChipType == CHIP_TYPE_MSG22XX)
    {
        if (pPacket[0] == 0xA7 && pPacket[1] == 0x00 && pPacket[2] == 0x08 && pPacket[3] == 0x60 && pPacket[4] == 0xFF && pPacket[5] == 0xFF && pPacket[6] == 0xFF)
        {
            DBG("ESD HwReset check : g_IsUpdateFirmware=%d\n", g_IsUpdateFirmware);
...
//begin 20161208 add by xiaobin.zhai for hardwareinfo
		tpd->tp_vendor_id = eSwId;
		tpd->tp_firmware_version = (nMajor<<12)+nMinor;
//end 20161208 add by xiaobin.zhai for hardwareinfo
...
s32 DrvFwCtrlDisableProximity(void)
{
    ...
    //Deleted by xiaobin.zhai for Task 4593652 on 2017/04/13 begin
//    mutex_lock(&g_Mutex);
	//Deleted by xiaobin.zhai for Task 4593652 on 2017/04/13 end
    mdelay(I2C_WRITE_COMMAND_DELAY_FOR_FIRMWARE);
    rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szDbBusTxData[0], 4);
    if (rc > 0)
    {
