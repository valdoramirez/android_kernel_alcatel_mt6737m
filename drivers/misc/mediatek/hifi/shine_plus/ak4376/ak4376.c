...
static struct audio_gpio_attr ak4376_gpios[GPIO_HIFI_LDO_END] = {
	[GPIO_HIFI_LDO_DIS] = {"hifi_ldo_low", false, NULL},
	[GPIO_HIFI_LDO_ENA] = {"hifi_ldo_high", false, NULL},
	[GPIO_HIFI_PWR_DWN] = {"hifi_pwr_d", false, NULL},
	[GPIO_HIFI_PWR_UP] = {"hifi_pwr_u", false, NULL},
};
...
#else
		akdbgprt("\t[AK4376] %s(%d)\n",__FUNCTION__,__LINE__);
		if(ak4376_gpios[GPIO_HIFI_PWR_UP].gpio_prepare){
			akdbgprt("\t[AK4376] %s(%d)\n",__FUNCTION__,__LINE__);
			ret = pinctrl_select_state(ak4376_pinctrl, ak4376_gpios[GPIO_HIFI_PWR_UP].gpioctrl);
			if(ret){
...
#else
		akdbgprt("\t[AK4376] %s(%d)\n",__FUNCTION__,__LINE__);
		if(ak4376_gpios[GPIO_HIFI_PWR_DWN].gpio_prepare){
			akdbgprt("\t[AK4376] %s(%d)\n",__FUNCTION__,__LINE__);
			ret = pinctrl_select_state(ak4376_pinctrl, ak4376_gpios[GPIO_HIFI_PWR_DWN].gpioctrl);
			if(ret){
...
//	if(ak4376_gpios[GPIO_HIFI_LDO_ENA].gpio_prepare){
//		akdbgprt("\t[AK4376] %s(%d)\n",__FUNCTION__,__LINE__);
//		ret = pinctrl_select_state(ak4376_pinctrl, ak4376_gpios[GPIO_HIFI_LDO_ENA].gpioctrl);
//		if(ret){
...
//	ak4376->priv_pdn_en = GPIO16_HIFI_PDN;
	...
	if (i2c->dev.platform_data)	{
		...
		ak4376->priv_pdn_en = ((struct ak4376_platform_data *)(i2c->dev.platform_data))->pdn_en;
		...
		if (gpio_is_valid(ak4376->priv_pdn_en)) {
			...
			if (ret2) {
				...
				ret2 = gpio_direction_output(ak4376->priv_pdn_en, 0);
				if (ret2) {
...
#if 0//defined(CONFIG_OF) && !defined(CONFIG_MTK_LEGACY)
