...
//static int sensor_suspend; // delete by binpeng.huang.hz@tcl.com
...
static int MXC400X_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	/* begin: modify by binpeng.huang.hz@tcl.com */
	//if (sensor_suspend == 1)
	if (atomic_read(&obj->suspend) == 1)
	{
		GSE_ERR("power error: device suspend\n");
		return 0;
	}
	/* end: modify by binpeng.huang.hz@tcl.com */
...
static int MXC400X_ReadRawData(struct i2c_client *client, char *buf)
{
	...
	/* begin: add by binpeng.huang.hz@tcl.com */
	if (atomic_read(&obj->suspend) == 1)
	{
		GSE_ERR("power error: device suspend\n");
		return 0;
	}
	/* end: add by binpeng.huang.hz@tcl.com */
...
static int mxc400x_resume(struct i2c_client *client)
{
	 /* begin: modify by binpeng.huang.hz */
	 struct mxc400x_i2c_data *obj = i2c_get_clientdata(client);		  
	...
	if (obj == NULL) {
	 ...
	 if(MXC400X_SetPowerMode(obj->client, true) != 0)
	 {
     ...
     mutex_unlock(&mxc400x_mutex);
	return 0;
	 /* end: modify by binpeng.huang.hz */
}
...
static int mxc400x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	GSE_FUN();
	client->addr = 0x15; /* add by binpeng.huang.hz@tcl.com on 2016.08.31 */
