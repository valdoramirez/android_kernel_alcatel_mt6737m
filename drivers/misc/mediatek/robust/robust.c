...
extern int nfc_init_flag;

static ssize_t nfc_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int status = 0;
...
static ssize_t nfc_status_store(struct device* dev, struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}
...
static const robust_node_info robust_node[MAX_ROBUST_NODE_NUM] = {
	{"NFC" , "status", nfc_status_show , nfc_status_store},
