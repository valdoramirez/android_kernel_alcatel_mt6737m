...
#include "s5k5e8yxx1mipiraw_Sensor.h"
#define S5k5e8_OTP
...
static void sensor_init(void)
{
   ...
   // +++++++++++++++++++++++++++//                                                               
	...
	// streaming ON
	//write_cmos_sensor(0x0100,0x01); //yzy
...
static int r_ratio=0;
static int b_ratio=0;
static void read_OTP(void)
{
	int page_flag =0;
	int page1_flag =0;
	int module_id=0,lens_id=0,vcm_id=0,driver_ic_id=0,year=0,month=0,date=0;
	int rg_ratio=0;
	int bg_ratio=0;
	int g_rg_ratio=0;
	int g_bg_ratio=0;
	...
	if(page_flag==0x01){
		...
		if((module_id==0xc)&&(lens_id==0x33)&&(vcm_id==0x0)&&(driver_ic_id==0x0))
		{
			OTP_vailiable=1;
		}
		else{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				sprintf(Front_Camera_Name,"S5k5e8yx:Sunrise:SUP2:5M_FF");
				if(OTP_vailiable==1){
...
UINT32 S5K5E8YXX1_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
