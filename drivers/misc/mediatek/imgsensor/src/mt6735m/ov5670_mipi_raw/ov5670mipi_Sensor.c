...
{
	read_otp(&ov5670_otp_data);
	if((otp_flag!=0)&&(otp_awb_flag!=0)&&(mid==0xc)&&(lens_id==0xb1)&&(vcm_id==0))
		{
			otp_flag_Validate=1;
	}
	else{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			if(vcm_id==0x0)
				{
				LOG_INF("ov5670 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
                                sprintf(Back_Camera_Name,"OV5670:Sunrise:SUP1:5M_FF");
