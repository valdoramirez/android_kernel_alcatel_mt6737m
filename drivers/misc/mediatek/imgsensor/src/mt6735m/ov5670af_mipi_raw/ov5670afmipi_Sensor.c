...
#include "ov5670afmipi_Sensor.h"

#define PFX "ov5670af_camera_sensor"
#define LOG_INF(format, args...)	pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

#define OV5670_OTP_CALI

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static int otp_flag_Validate=0,otp_flag_Validate_mini=0,otp_flag=0,otp_awb_flag=0,mid=0,lens_id=0,vcm_id=0,check_awb_data=0;
#ifndef ABS
...
static struct otp_struct ov5670_otp_data;
...
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	printk("[ov5670 otp]temp2=%d,temp3=%d\n",temp2,temp3);
...
static int apply_otp(struct otp_struct *otp_ptr)
{
	...
	if ((*otp_ptr).flag & 0x40) {
		...
		printk("rg=%x, bg=%x, R_gain=%x, B_gain=%x, G_gain=%x\n", rg,bg,R_gain,B_gain,G_gain);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x450b, 0x20); //Defect 415372 modify by yinzhanyong at 20150706
...
#ifdef OV5670_OTP_CALI
	//ov5670_read_otp();
	read_otp(&ov5670_otp_data);
	apply_otp(&ov5670_otp_data);
#endif
}	/*	sensor_init  */
...
static void Check_OTP(void)
{
	read_otp(&ov5670_otp_data);
	if((otp_flag!=0)&&(otp_awb_flag!=0)&&(mid==0xc)&&(lens_id==0xb1)&&(vcm_id==0x63))
		{
			otp_flag_Validate=1;
	}
	else{
	...
	if((otp_flag!=0)&&(otp_awb_flag!=0)&&(mid==0xc)&&(lens_id==0xb1))
		{
			otp_flag_Validate_mini=1;
	}
	else{
			otp_flag_Validate_mini=0;
		}
	printk("otp_flag_Validate=%d\n",otp_flag_Validate);
	printk("otp_flag_Validate=%d\n",otp_flag_Validate_mini);
}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			if(vcm_id==0x63)
				{
				LOG_INF("ov5670af i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
                                sprintf(Back_Camera_Name,"OV5670:Sunrise:SUP1:5M_AF");
					    if(otp_flag_Validate_mini==1){
