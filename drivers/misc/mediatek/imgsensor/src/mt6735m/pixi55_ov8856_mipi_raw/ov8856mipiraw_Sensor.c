...
//#define OV8856_OTP_CALI

#ifdef OV8856_OTP_CALI
#define OTP_LSC_SIZE 240
#define RG_Ratio_Typical   635
#define BG_Ratio_Typical   630
extern char g_main_camera_otp;
...
struct otp_struct {
	...
	u16 VCM_end;
	int VCM_dir;
	unsigned char lsc[OTP_LSC_SIZE];
	int checksum;
};
static struct otp_struct ov8856_otp_data;

static int read_otp(struct otp_struct *otp_ptr);
static int apply_otp(struct otp_struct *otp_ptr);
static int otp_flag_Validate = 0;
int ov8856_afotp_validflag = 0;
u16 ov8856_afmacro = 0;
u16 ov8856_afinf = 0;
#endif
...
//#define LOG_INF(format, args...)	printk(4 ,PFX, "[%S] " format, __FUNCTION__, ##args)
...
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	if(addr != 0) {
		(*otp_ptr).flag = 0x01; // valid info and AWB in OTP
		...
		(*otp_ptr).driver_ic = read_cmos_sensor( addr + 3);
		...
		LOG_INF("invalid module_info in OTP\n");
	}
	...
	addr = 0;
	if((otp_flag & 0xC0) == 0x40) {
		addr = 0x7020; // base address of info group 1
	}
	else if((otp_flag & 0x30) == 0x10) {
		addr = 0x7023; // base address of info group 2
	}
	if(addr != 0) {
		(*otp_ptr).flag |= 0x04;
	...
	// OTP VCM Calibration
	otp_flag = read_cmos_sensor(0x7026);

	addr = 0;
	if((otp_flag & 0xC0) == 0x40) {
		addr = 0x7027; // base address of VCM Calibration group 1
	}
	else if((otp_flag & 0x30) == 0x10) {
		addr = 0x702A; // base address of VCM Calibration group 2
	}
	if(addr != 0) {
		(*otp_ptr).flag |= 0x10;
		temp = read_cmos_sensor(addr + 2);
		...
		if(((* otp_ptr).VCM_start !=0) && ((* otp_ptr).VCM_end !=0))
		{
			ov8856_afinf = (* otp_ptr).VCM_start;
			ov8856_afmacro = (* otp_ptr).VCM_end;
			ov8856_afotp_validflag = 1;
		}
	}
	else {
		(* otp_ptr).VCM_start = 0;
		(* otp_ptr).VCM_end = 0;
		(* otp_ptr).VCM_dir = 0;
		ov8856_afotp_validflag = 0;
		LOG_INF("invalid VCM Calibration in OTP\n");
	}
	LOG_INF("AFMacro = %d, AFInf = %d, dir = %d \n", (* otp_ptr).VCM_start, (* otp_ptr).VCM_end, (* otp_ptr).VCM_dir);
	// OTP lsc Calibration
	otp_flag = read_cmos_sensor(0x702D);

	addr = 0;
	
	if((otp_flag & 0xC0) == 0x40) {
		addr = 0x702E; // base address of Lenc Calibration group 1
	}
	else if((otp_flag & 0x30) == 0x10) {
		addr = 0x711F; // base address of Lenc Calibration group 2
	}
	if(addr != 0) {
		for(i = 0; i < OTP_LSC_SIZE; i++) {
			(* otp_ptr).lsc[i]=read_cmos_sensor(addr + i);
			checksum2 += (* otp_ptr).lsc[i]; 
			//LOG_INF("(* otp_ptr).lenc[%d]=%x ,checksum2=%x\n",i,(* otp_ptr).lenc[i],checksum2);
		}
		checksum2 = (checksum2)%255 +1;
		(* otp_ptr).checksum = read_cmos_sensor(addr + OTP_LSC_SIZE);
	...
	if(((*otp_ptr).flag == 0x55))//&& ((* otp_ptr).module_integrator_id==0x45) && ((* otp_ptr).lens_id==0x73) && ((* otp_ptr).vcm_id==0))
	{
		otp_flag_Validate = 1;
		g_main_camera_otp = 1;
	}
...
static int apply_otp(struct otp_struct *otp_ptr)
{
	...
	if ((*otp_ptr).flag & 0x40) {
		...
		for(i = 0; i < OTP_LSC_SIZE; i++) {
			write_cmos_sensor(0x5900 + i, (*otp_ptr).lsc[i]);
...
static kal_uint32 open(void)
{
	...
	mdelay(10);
#ifdef OV8856_OTP_CALI
	if(otp_flag_Validate != 1)
	{
		read_otp(&ov8856_otp_data);
	}
	if(otp_flag_Validate == 1)
	{
		apply_otp(&ov8856_otp_data);
	}
#endif
	spin_lock(&imgsensor_drv_lock);
