...
#include "ov13850smipiraw_Sensor.h"
#include "ov13850s_otp.h"
...
kal_uint16 OV13850_R2A_write_i2c(u16 addr,u32 para)
{
...
void otp_cali(unsigned char writeid)
{
	...
	printk("OV13850MIPI_WRITE_ID is 0x%x",OV13850MIPI_WRITE_ID);
...
int ov13850_MAC_MSB=0,ov13850_INF_MSB=0,ov13850_LSB=0;
int read_otp(struct otp_struct *otp_ptr)
{
	...
	if((otp_flag & 0xc0) == 0x40) {
		...
		addr = 0x722f; // base address of info group 2
	}
	if(addr != 0) {
		awb_flag=1;
		(*otp_ptr).flag = 0xC0; // valid info and AWB in OTP
		(*otp_ptr).module_integrator_id = OV13850_R2A_read_i2c(addr);
		(*otp_ptr).lens_id = OV13850_R2A_read_i2c( addr + 4);
		(*otp_ptr).production_year = OV13850_R2A_read_i2c( addr + 1);
		(*otp_ptr).production_month = OV13850_R2A_read_i2c( addr + 2);
		(*otp_ptr).production_day = OV13850_R2A_read_i2c(addr + 3);
		(*otp_ptr).vcm_id = OV13850_R2A_read_i2c( addr + 5);
		(*otp_ptr).vcm_ic_id = OV13850_R2A_read_i2c( addr + 6);
		temp = OV13850_R2A_read_i2c(addr + 9);
		(*otp_ptr).rg_ratio = (OV13850_R2A_read_i2c(addr + 7)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).bg_ratio = (OV13850_R2A_read_i2c(addr + 8)<<2) + ((temp>>4) & 0x03);	
		temp2=OV13850_R2A_read_i2c(addr + 12);
		//(*otp_ptr).g_rg_ratio = (OV13850_R2A_read_i2c(addr + 10)<<2) + ((temp2>>6) & 0x03);
		//(*otp_ptr).g_bg_ratio = (OV13850_R2A_read_i2c(addr + 11)<<2) + ((temp2>>4) & 0x03);
		(*otp_ptr).g_rg_ratio =0x118;
		(*otp_ptr).g_bg_ratio=0x132;
		(*otp_ptr).checksum=OV13850_R2A_read_i2c(addr + 13);
		LOG_INF("Golden_AWB_RG_MSB =0x%x,Golden_AWB_BG_MSB =0x%x,Golden_AWB_LSB=0x%x\n",OV13850_R2A_read_i2c(addr + 10),OV13850_R2A_read_i2c(addr + 11),temp2);
	}
	else {
	...
	for(i=0;i<=12;i++){
	checksum+=OV13850_R2A_read_i2c(addr+i);
	}
	checksum=(checksum)%255 +1;
	LOG_INF("module_integrator_id=0x%x,lens_id=0x%x,data=%d-%d-%d\n",(*otp_ptr).module_integrator_id,(*otp_ptr).lens_id,(*otp_ptr).production_year,(*otp_ptr).production_month,(*otp_ptr).production_day);
	LOG_INF("vcm_id:0x%x,vcm_ic_id:0x%x\n",(*otp_ptr).vcm_id,(*otp_ptr).vcm_ic_id);
	LOG_INF("rg_ratio=0x%x,bg_ratio=0x%x\n",(*otp_ptr).rg_ratio,(*otp_ptr).bg_ratio);
	LOG_INF("g_rg_ratio=0x%x,g_bg_ratio=0x%x\n",(*otp_ptr).g_rg_ratio,(*otp_ptr).g_bg_ratio);
	LOG_INF("(*otp_ptr).checksum =%d\n",(*otp_ptr).checksum);
	LOG_INF("checksum =%d\n",checksum);
	// OTP VCM Calibration
	otp_flag = OV13850_R2A_read_i2c(0x73b8);
	LOG_INF(" VCM calibration data : %x \n", otp_flag);
	addr = 0;
	if((otp_flag & 0xc0) == 0x40) {
		addr = 0x73b9; // base address of VCM Calibration group 1
	}
	else if((otp_flag & 0x30) == 0x10) {
		addr = 0x73bc; // base address of VCM Calibration group 2
	}
	if(addr != 0) {
		af_flag=1;
		(*otp_ptr).flag |= 0x20;
		...
		ov13850_MAC_MSB=OV13850_R2A_read_i2c(addr);
		ov13850_INF_MSB=OV13850_R2A_read_i2c(addr+1);
		ov13850_LSB=OV13850_R2A_read_i2c(addr+2);
	}
	else {
		(* otp_ptr).VCM_start = 0;
		(* otp_ptr).VCM_end = 0;
	...
	LOG_INF("VCM start :%d,VCM end:%d,VCM dir:%d\n",(* otp_ptr).VCM_start,(* otp_ptr).VCM_end,(* otp_ptr).VCM_dir);
	// OTP Lenc Calibration
	LOG_INF("ov13850_MAC_MSB:0x%x,ov13850_INF_MSB:0x%x,ov13850_LSB:0x%x\n",ov13850_MAC_MSB,ov13850_INF_MSB,ov13850_LSB);
	otp_flag = OV13850_R2A_read_i2c(0x723d);
	...
	if((otp_flag & 0xc0) == 0x40) {
		...
		addr = 0x72fb; // base address of Lenc Calibration group 2
	}
	//
...
if(addr != 0) {
	...
	if((awb_flag==1)&&(af_flag==1)&&(lsc_flag==1)&&(checksum==(*otp_ptr).checksum)&&((*otp_ptr).module_integrator_id==0x6)&&((*otp_ptr).lens_id==0x2)&&((*otp_ptr).vcm_id==0x64)&&((*otp_ptr).vcm_ic_id==0x12))
	{
		OV13850_OTP_FLAG=1;
	}
	else{
...
int apply_otp(struct otp_struct *otp_ptr)
{
	...
	if ((*otp_ptr).flag & 0x40) {
		...
		B_gain = (g_bg*1000) / bg;
		G_gain = 1000;
		LOG_INF("rg:0x%x,bg:0x%x,g_rg:0x%x,g_bg:0x%x\n",rg,bg,g_rg,g_bg);
