...
static int otp_flag_Validate=0,otp_flag=0,otp_awb_flag=0,mid=0,lens_id=0,vcm_id=0,check_awb_data=0;
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5675AF_MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
*/
#ifdef OV5675_OTP_CALI
//static int rg_h,bg_h,rg_bg_l,g_ave1;
// return value:
// bit[7]: 0 no otp info, 1 valid otp info
// bit[6]: 0 no otp wb, 1 valib otp wb
static int read_otp(struct otp_struct *otp_ptr)
{
	int addr=0, temp=0, i=0;
	int temp1=0,temp2=0,temp3=0;
	//set 0x5002[3] to \930\94
	...
	LOG_INF("TCL OV5675 OTP info otp_flag:0x%x,module_integrator_id:0x%x,lens_id:0x%x,vcm_id:0x%x,vcm_ic_id:0x%x,date is %d-%d-%d\n",otp_flag,(*otp_ptr).module_integrator_id,(*otp_ptr).lens_id,(*otp_ptr).vcm_id,(*otp_ptr).vcm_ic_id,(*otp_ptr).production_year,(*otp_ptr).production_month,(*otp_ptr).production_day);
	...
	LOG_INF("[ov5675 otp]temp2=%d,temp3=%d\n",temp2,temp3);
	...
	LOG_INF("[ov5675 otp]rg_h = %x, rg_l = %x, bg_h = %x, bg_l = %x\n", read_cmos_sensor(addr + 1), ((temp>>6) & 0x03), read_cmos_sensor( addr + 2), ((temp>>4) & 0x03));
	LOG_INF("[ov5675 otp]rg = %x, bg = %x,g_rg=%x,g_bg=%x\n", (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio,(*otp_ptr).g_rg_ratio,(*otp_ptr).g_bg_ratio);
	...
	if((otp_flag!=0)&&(otp_awb_flag!=0))//&&(mid==0x45)&&(lens_id==0x73)&&(vcm_id==0))
	{
		otp_flag_Validate = 1;
		g_main_camera_otp = 1;
	}
...
static int apply_otp(struct otp_struct *otp_ptr)
{
	...
	if ((*otp_ptr).flag & 0x40) {
		...
		LOG_INF("rg=%x, bg=%x, R_gain=%x, B_gain=%x, G_gain=%x\n", rg,bg,R_gain,B_gain,G_gain);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	kal_uint32 tem_sensor_id=0x5675; 
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == tem_sensor_id) {				
#ifdef OV5675_OTP_CALI
				read_otp(&ov5675_otp_data);
#endif
		if(lens_id!=0xb5){
		LOG_INF("[OV5675 AF]not ov5675 AF module ! \n");
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
			}
				*sensor_id=imgsensor_info.sensor_id;
				LOG_INF("[OV5675 AF]i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			LOG_INF("[OV5675 AF]Read sensor id: 0x%x fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	kal_uint16 tem_sensor_id=0x5675;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		if (lens_id!=0xb5){
			LOG_INF("[OV5675 AF] not the OV5675 AF module! \n");
			return ERROR_SENSOR_CONNECT_FAIL;
			}
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
...
UINT32 OV5675_AF_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
