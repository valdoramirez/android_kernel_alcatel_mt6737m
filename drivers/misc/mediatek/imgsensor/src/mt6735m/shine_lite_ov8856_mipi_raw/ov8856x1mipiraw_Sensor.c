...
#include "ov8856x1mipiraw_Sensor.h"
...
static struct otp_struct1 otp_ptr;
static bool otp_read_flag = FALSE;
...
static kal_uint32 open(void)
{
	...
	*/
	...
	if(ERROR_NONE !=get_imgsensor_id(&sensor_id))
		return ERROR_SENSOR_CONNECT_FAIL;
...
UINT32 OV8856X1_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
...
static int read_otp(struct otp_struct1 *otp_ptr)
{
...
static int apply_otp(struct otp_struct1 *otp_ptr)
{
...
/*
static int read_module_id(void)
{
	int otp_flag, addr, i;
	int module_integrator_id, lens_id;
	//set 0x5001[3] to ???0??¨¤
	int temp1;
*/	
	/* initail sequence write in  */
/*	sensor_init();
	...
	if(addr != 0) {
	//(*otp_ptr).flag = 0xC0; // valid info and AWB in OTP
	module_integrator_id = read_cmos_sensor(addr);
	...
	temp1 = read_cmos_sensor(0x5001);
	write_cmos_sensor(0x5001, (0x08 & 0x08) | (temp1 & (~0x08)));
	LOG_INF("module_integrator_id=%x \n",module_integrator_id);
