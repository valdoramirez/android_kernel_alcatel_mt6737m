...
#include "ov5675x1mipi_Sensor.h"

#define PFX "ov5675x1_camera_sensor"
...
{
	read_otp(&ov5675_otp_data);
	if((otp_flag!=0)&&(otp_awb_flag!=0)&&(mid==0x44)&&(lens_id==0x72)&&(vcm_id==0))
		{
			otp_flag_Validate=1;
	}
	else{
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				sprintf(Front_Camera_Name,"OV5675:E-WELLY:SUP1:5M_FF");
...
UINT32 OV5675X1_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
