...
static void sensor_init(void)
{
  ...
  write_cmos_sensor(0x6F12, 0x0000);  //ob for full size
...
//	otp_lsc_update();
  ...
  //e 2014/12/01, bruce
  LOG_INF("sensor init 0x0002 value is 0x%x\n",read_cmos_sensor(0x0002));
}	/*	sensor_init  */
/**********************************************************************************************************************/
...
// e 2014/12/01, bruce
LOG_INF("sensor preview 0x0002 value is 0x%x\n",read_cmos_sensor(0x0002));
}	/*	preview_setting  */
/**********************************************************************************************************************/
...
// s bruce 2014/12/01
	...
	write_cmos_sensor(0x0308,0x0008);
	write_cmos_sensor(0x0342,0x1CF0);
	write_cmos_sensor(0x0340,0x04B0);
	write_cmos_sensor(0x0202,0x0200);
	write_cmos_sensor(0x0200,0x0400);
	write_cmos_sensor_8(0x0B05,0x01  );
	write_cmos_sensor_8(0x0B08,0x01  );
...
//s bruce, 2014/12/01
	...
	write_cmos_sensor(0x32CA,0x0449);
	write_cmos_sensor(0x32D6,0x0449);
	write_cmos_sensor(0x0344,0x01FC);
	write_cmos_sensor(0x0346,0x0250);
	write_cmos_sensor(0x0348,0x0E83);
	write_cmos_sensor(0x034A,0x09EF);
	write_cmos_sensor(0x034C,0x0320);
	...
	write_cmos_sensor(0x0308,0x0008);
	write_cmos_sensor(0x0342,0x1CF0);
	write_cmos_sensor(0x0340,0x0384);
	write_cmos_sensor(0x0202,0x0200);
	write_cmos_sensor(0x0200,0x0400);
	write_cmos_sensor_8(0x0B05,0x01  );
	write_cmos_sensor_8(0x0B08,0x01  );
...
#ifdef s5k3m2_otp
static int S5K3M2_OTP_FLAG=0;
int s5k3m2_MAC_MSB=0,s5k3m2_INF_MSB=0,s5k3m2_MAC_LSB=0,s5k3m2_INF_LSB=0;
int s5k3m2_RG_MSB=0,s5k3m2_BG_MSB=0,s5k3m2_RG_LSB=0,s5k3m2_BG_LSB=0;
int s5k3m2_gRG_MSB=0,s5k3m2_gBG_MSB=0,s5k3m2_gRG_LSB=0,s5k3m2_gBG_LSB=0;
static void s5k3m2_read_otp(void)
{
int i=0;
int addr=0;
...
write_cmos_sensor(0x0A02,0x1F00);
write_cmos_sensor(0x0A00,0x0100);
mdelay(5);
for(i=0;i<57;i++)
{
...
printk("TCL s5k3m2 OTP addr:0A04+%d,value:0x%x\n",i,temp);
}
//awb otp
awb_flag=read_cmos_sensor_8(0x0A04);
awb_flag1=read_cmos_sensor_8(0x0A22);
if(awb_flag==0x1)
{
printk("TCL s5k3m2 AWB OTP group1 is valid\n");
addr=0x0A04;
...
s5k3m2_RG_MSB=read_cmos_sensor_8(addr+10);
s5k3m2_RG_LSB=read_cmos_sensor_8(addr+11);
s5k3m2_BG_MSB=read_cmos_sensor_8(addr+12);
s5k3m2_BG_LSB=read_cmos_sensor_8(addr+13);

s5k3m2_gRG_MSB=read_cmos_sensor_8(addr+14);
s5k3m2_gRG_LSB=read_cmos_sensor_8(addr+15);
s5k3m2_gBG_MSB=read_cmos_sensor_8(addr+16);
s5k3m2_gBG_LSB=read_cmos_sensor_8(addr+17);
for(i=0;i<20;i++){
checksum1+=read_cmos_sensor_8(0x0A06+i);
}
checksum1=(checksum1)%255+1;
printk("TCL s5k3m2 OTP module_id is 0x%x,lens_id is 0x%x,vcm_id is 0x%x,vcm_ic_id is 0x%x\n",module_id,lens_id,vcm_id,vcm_ic_id);
printk("TCL s5k3m2 OTP date is %d-%d-%d\n",year,month,date);
printk("TCL s5k3m2 OTP rg_ratio:0x%x,bg_ratio:0x%x\n",rg_ratio,bg_ratio);
printk("TCL s5k3m2 OTP g_rg_ratio:0x%x,g_bg_ratio:0x%x\n",g_rg_ratio,g_bg_ratio);
printk("TCL s5k3m2 OTP checksum is %d,checksum1 is %d\n",checksum,checksum1);
}
else if(awb_flag1==0x1){
printk("TCL s5k3m2 AWB OTP group2 is valid\n");
addr=0x0A22;
checksum=read_cmos_sensor_8(addr+1);
module_id=read_cmos_sensor_8(addr+2);
year=read_cmos_sensor_8(addr+3);
...
printk("TCL s5k3m2 AWB OTP is invalid\n");
}
//af otp
af_flag=read_cmos_sensor_8(0x0A1A);
af_flag1=read_cmos_sensor_8(0x0A34);
if(af_flag==0x1){
printk("TCL s5k3m2 AF OTP group1 is valid\n");
checksumaf=read_cmos_sensor_8(0x0A1B);
macro=(read_cmos_sensor_8(0x0A1C)<<8)|(read_cmos_sensor_8(0x0A1D)&0xff);
inf=(read_cmos_sensor_8(0x0A1E)<<8)|(read_cmos_sensor_8(0x0A1F)&0xff);
s5k3m2_MAC_MSB=read_cmos_sensor_8(0x0A1C);
s5k3m2_MAC_LSB=read_cmos_sensor_8(0x0A1D);
s5k3m2_INF_MSB=read_cmos_sensor_8(0x0A1E);
s5k3m2_INF_LSB=read_cmos_sensor_8(0x0A1F);
for(i=0;i<4;i++){
checksumaf1+=read_cmos_sensor_8(0x0A1C+i);
}
checksumaf1=(checksumaf1)%255+1;
printk("TCL s5k3m2 OTP checksumaf is %d,checksumaf1 is %d\n",checksumaf,checksumaf1);
printk("TCL s5k3m2 OTP macro is %d,inf is %d\n",macro,inf);
}
else if(af_flag1==0x1){
printk("TCL s5k3m2 AF OTP group2 is valid\n");
...
s5k3m2_MAC_MSB=read_cmos_sensor_8(0x0A36);
s5k3m2_MAC_LSB=read_cmos_sensor_8(0x0A37);
s5k3m2_INF_MSB=read_cmos_sensor_8(0x0A38);
s5k3m2_INF_LSB=read_cmos_sensor_8(0x0A39);
for(i=0;i<4;i++){
...
printk("TCL s5k3m2 AF OTP is invalid\n");
}
if(((af_flag==0x1)||(af_flag1==0x1))&&(module_id==0x1)&&(lens_id==0x62)&&(vcm_id==0xb3)&&(vcm_ic_id==0x12)&&(checksum==checksum1)&&(checksumaf==checksumaf1)){
	S5K3M2_OTP_FLAG=1;
}
else{
	S5K3M2_OTP_FLAG=0;
}
printk("TCL s5k3m2 OTP S5K3M2_OTP_FLAG is %d",S5K3M2_OTP_FLAG);
write_cmos_sensor_8(0x0A00,0x00);
}
void s5k3m2_apply_lsc_otp(void)
{
	write_cmos_sensor(0x6028,0x2000);
	write_cmos_sensor(0x602A,0x14FA);
	write_cmos_sensor_8(0x6F12,0x0F);
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12);
			...
			s5k3m2_read_otp();
					...
					sprintf(Back_Camera_Name,"S5K3M2:Sunny:SUP2:13M_AF"); 
					if(S5K3M2_OTP_FLAG==1){
...
static kal_uint32 open(void)
{
	...
	sensor_init();
	#ifdef s5k3m2_otp
      s5k3m2_read_otp();//add by zhanyong.yin
      s5k3m2_apply_lsc_otp();
	 #endif
	spin_lock(&imgsensor_drv_lock);
...
UINT32 S5K3M2S_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
