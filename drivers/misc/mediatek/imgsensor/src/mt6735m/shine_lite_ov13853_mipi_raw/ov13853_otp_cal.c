...
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	if((otp_flag==0x01)&&(awb_flag==0x01)&&(lsc_flag==0x01)&&((*otp_ptr).module_integrator_id==0x6)&&((*otp_ptr).lens_id==0x12)&&((*otp_ptr).vcm_id==0x62)&&((*otp_ptr).vcm_ic_id==0xe)&&(checksum==(*otp_ptr).checksumOTP)&&(checksumAWB==(*otp_ptr).checksumAWB))
	{
			OV13853_OTP_Flag=1;
	}
	else{
