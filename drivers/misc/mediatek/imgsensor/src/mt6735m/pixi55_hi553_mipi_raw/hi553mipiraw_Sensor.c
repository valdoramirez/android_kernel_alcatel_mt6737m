...
#define HI553_OTP_USE
#ifdef HI553_OTP_USE
extern bool hi553_otp_update(void);
extern bool hi553_wb_gain_set(void);
bool hi553_my_otp_flag=0 ;
...
static kal_uint32 open(void)
{
	...
	#ifdef HI553_OTP_USE
		if(hi553_my_otp_flag == 1)
		{
			hi553_wb_gain_set();
			printk("*****hi553 update otp*******\n");		
		}
		if(hi553_my_otp_flag==0)
		{
			printk("**********hi553 enter otp update********\n");	
			if(hi553_otp_update())
            {
                g_main_camera_otp = 1;
                hi553_my_otp_flag = 1;
