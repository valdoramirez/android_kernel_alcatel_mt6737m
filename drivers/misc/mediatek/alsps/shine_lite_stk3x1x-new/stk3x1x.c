...
#define PSCTRL_VAL	0x21	/* ps_persistance=4, ps_gain=64X, PS_IT=0.391ms */
#define ALSCTRL_VAL	0x3A	/* als_persistance=1, als_gain=64X, ALS_IT=50ms */
...
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	170
	#define STK_LT_N_CT	120  //
	#define STK_HT_N_CT	165 //100
...
struct alsps_hw alsps_cust; //modify by lanying.he static struct alsps_hw alsps_cust;
...
static void stk_ps_tune0_offset(struct stk3x1x_priv *obj,int data);
#endif
#ifdef STK_CHK_REG	
...
//#ifdef STK_TUNE0
	...
	if(enable)
	{
		...
		if(!(old & STK_STATE_EN_ALS_MASK))
			cur |= STK_STATE_EN_WAIT_MASK;
		if(0 == obj->hw->polling_mode_ps)  //modify by lanying.he 
		{
		//	APS_LOG("%s: line=%d\n", __FUNCTION__, __LINE__);
			wake_lock(&ps_lock);
		}
		...
		else
		{
			...
			{			
				...
				{	
					...
					sensor_data.value_divide = 1;
					sensor_data.status = SENSOR_STATUS_ACCURACY_MEDIUM;
					APS_LOG("%s:linhui ps raw 0x%x -> value 0x%x \n",__FUNCTION__, obj->ps,
									sensor_data.values[0]);
					APS_LOG("%s: linhui line=%d\n", __FUNCTION__, __LINE__);
...
static int stk_ps_tune_zero_final(struct stk3x1x_priv *obj)
{
	...
	stk_ps_tune0_offset(obj,obj->psi);
#ifndef CALI_EVERY_TIME
...
#else						
	obj->ps_high_thd_boot = obj->ps_stat_data[2] + obj->stk_ht_n_ct;
	obj->ps_low_thd_boot = obj->ps_stat_data[2] + obj->stk_lt_n_ct;
	atomic_set(&obj->ps_threshold_high, obj->ps_high_thd_boot); 
	atomic_set(&obj->ps_threshold_low, obj->ps_low_thd_boot); 
#endif
	...
	atomic_set(&obj->last_ct,obj->ps_stat_data[2]);
	if(atomic_read(&obj->ps_threshold_low) < 400)
	{
		obj->boot_cali = 1;
	}	
	APS_LOG("%s: set HT=%d,LT=%d\n", __func__, atomic_read(&obj->ps_threshold_high),  atomic_read(&obj->ps_threshold_low));		
...
static void stk_ps_tune0_offset(struct stk3x1x_priv *obj,int data)
{
	int raw_data;
	raw_data = data;
	if((raw_data > 0) && (raw_data <= 80))
	{
		obj->stk_ht_n_ct = 40;
		obj->stk_lt_n_ct = 15; 
		obj->stk_max_min_diff = 40;
			}
	else if((raw_data > 80) && (raw_data <=110))
	{
		obj->stk_ht_n_ct = 60;
		obj->stk_lt_n_ct = 20;
		obj->stk_max_min_diff = 60;
	}	
	else if((raw_data > 110) && (raw_data <= 200))
	{
		obj->stk_ht_n_ct = 100;
		obj->stk_lt_n_ct = 25;
		obj->stk_max_min_diff = 100;
	}
	else 
	{
		obj->stk_ht_n_ct = 110;
		obj->stk_lt_n_ct = 50;
		obj->stk_max_min_diff = 110;
	}
}
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if(obj->psi_set)
	{
		//if((abs(word_data - atomic_read(&obj->last_ct)) < 5) && word_data < atomic_read(&obj->last_ct))
		if( word_data < atomic_read(&obj->last_ct) && ((atomic_read(&obj->ps_nf_flag)) == 1))
		{
		        APS_ERR("%s:obj->ps_nf_flag=%d\n",__func__,atomic_read(&obj->ps_nf_flag));
				stk_ps_tune0_offset(obj,word_data);
		    	...
		    	atomic_set(&obj->last_ct,word_data);
			APS_LOG("%s: set HT=%d, LT=%d ojb->psi_set = %d\n", __func__, atomic_read(&obj->ps_threshold_high), atomic_read(&obj->ps_threshold_low),obj->psi_set);
		}
...
#ifdef CALI_EVERY_TIME
			if( (atomic_read(&obj->ps_threshold_high) > (obj->ps_high_thd_boot + 200)) && (obj->boot_cali == 1) )
			{
				atomic_set(&obj->ps_threshold_high, obj->ps_high_thd_boot); 
				atomic_set(&obj->ps_threshold_low, obj->ps_low_thd_boot); 
...
#endif					
			APS_LOG("%s: set HT=%d, LT=%d ojb->psi_set = %d\n", __func__, atomic_read(&obj->ps_threshold_high), atomic_read(&obj->ps_threshold_low),obj->psi_set);
			atomic_set(&obj->last_ct,obj->psi);
			//hrtimer_cancel(&obj->ps_tune0_timer);
		}
	}
	return 0;
...
static void stk3x1x_eint_work(struct work_struct *work)
{
	...
	if(((1<<STK_BIT_PS) &  obj->pending_intr) && (obj->hw->polling_mode_ps == 0))
	{
		...
		atomic_set(&obj->ps_nf_flag,sensor_data.values[0]);
		if((0 == sensor_data.values[0]) && (obj->ps > (obj->psi*50/10)))
		{
			atomic_set(&obj->ps_threshold_high, obj->psi + 4 * obj->stk_ht_n_ct); 
			atomic_set(&obj->ps_threshold_low, obj->psi + 4 * obj->stk_lt_n_ct); 
			if((ret = stk3x1x_write_ps_high_thd(obj->client, atomic_read(&obj->ps_threshold_high))))
			{
				APS_ERR("write high thd error: %d\n", ret);
				goto err_i2c_rw;        
			...
			atomic_set(&obj->last_ct,obj->ps);
			obj->cali_status = true;
			//obj->ps_nf_flag = 0;
			APS_ERR("%s: set HT=%d, LT=%d,obj->cali_status = %d, obj->ps = %d\n", __func__, atomic_read(&obj->ps_threshold_high), atomic_read(&obj->ps_threshold_low),obj->cali_status,obj->ps);
		}
		else if(1 == sensor_data.values[0] && obj->cali_status == true)
		{
			...
			obj->cali_status = false;
			atomic_set(&obj->last_ct,obj->ps);
			//obj->ps_nf_flag = 1;
					APS_LOG("%s: set HT=%d, LT=%d obj->cali_status = %d obj->ps = %d\n", __func__, atomic_read(&obj->ps_threshold_high), atomic_read(&obj->ps_threshold_low),obj->cali_status,obj->ps);
		}
		sensor_data.value_divide = 1;
		sensor_data.status = SENSOR_STATUS_ACCURACY_MEDIUM;
	...
	APS_LOG("%s: lanying.he HT=%d, LT=%d\n", __func__, atomic_read(&obj->ps_threshold_high), atomic_read(&obj->ps_threshold_low));
...
static ssize_t stk3x1x_show_ps(struct device_driver *ddri, char *buf)
{
	...
	{
		//return scnprintf(buf, PAGE_SIZE, "0x%04X\n", stk3x1x_obj->ps);     
		return scnprintf(buf, PAGE_SIZE, "%d;thresh_near:%d;tresh_far:%d\n", stk3x1x_obj->ps,\
