...
#include "stk3x1x.h"
#define DRIVER_VERSION          "3.7.0_20160420_fin_det_pscomp"
...
/*----------------------------------------------------------------------------*/
	...
	#define STK_TUNE1_INVALID_PS_LIMIT	(5)
		...
		#undef STK_POLL_ALS
	#endif
	#define STK_FT_AVE_DARK_60DEG		188
	#define STK_FT_AVE_DARK_40DEG		47
	#define STK_FT_AVE_DARK_25DEG		14
	#define STK_PS0OFF_TOO_LARGE_THD	2500				
#endif
...
/*----------------------------------------------------------------------------*/
#ifdef STK_TUNE2	
static int32_t stk3x1x_read_otp26(struct i2c_client *client)	
{
	return stk3x1x_otp_read_byte_data(client, 0x26);	
}
#endif
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
#ifdef STK_TUNE2
static int32_t stk3x1x_get_dark_reading(struct i2c_client *client)
{	
	// struct stk3x1x_priv *obj = i2c_get_clientdata(client);    
	int ret = 0;
	u8 buf[2];
	...
	ret = stk3x1x_master_recv(client, 0x2A, buf, 0x02);
	if(ret < 0)
	{
		APS_DBG("error: %d\n", ret);
		return -EFAULT;
...
/*----------------------------------------------------------------------------*/
static void stk_ps_report(int nf)
{
...
#define REDUCE_RATIO_BASE	10000
#define MAX_LED_DUTY		0x3F

void stk_ps_tune2_normalize_dark(uint8_t alsctrl, int32_t *dark)
{
	uint8_t para;
...
/*----------------------------------------------------------------------------*/
void stk_ps_tune2_normalize_psoff(uint8_t psctrl, int32_t *psoff)
{
	switch(psctrl & 0x0f)
	{
	case 0x0:
...
/*----------------------------------------------------------------------------*/
static int32_t stk3x1x_get_psoff_reading(struct stk3x1x_priv *obj)
{	
	int ret = 0;
	u8 buf[2];
		
	ret = stk3x1x_master_recv(obj->client, 0x20, buf, 0x02);
	if(ret < 0)
	{
		APS_DBG("error: %d\n", ret);
		return -EFAULT;
...
/*----------------------------------------------------------------------------*/
static int32_t stk_ps_tune2_adjust_def_duty_it(struct stk3x1x_priv *obj)
{
	int ret = 0;

	if((ret = stk3x1x_write_ps(obj->client, atomic_read(&obj->psctrl_val))))
	{
		APS_ERR("write ps error: %d\n", ret);
		return ret;        
	}

	if((ret = stk3x1x_write_led(obj->client, obj->ledctrl_val)))
	{
		APS_ERR("write ps error: %d\n", ret);
		return ret;        
	}	
	printk("%s: reg=0x%x,0x%x\n", __func__, atomic_read(&obj->psctrl_val), obj->ledctrl_val);	
	return 0;
}
/*----------------------------------------------------------------------------*/
static int32_t stk_ps_tune2_adjust_duty_it(struct stk3x1x_priv *obj, uint16_t reduced_ratio)
{
	uint8_t tar_psctrl_reg, tar_ledctrl_reg;
	uint32_t temp_para;
	int ret;		
	
	if(reduced_ratio == REDUCE_RATIO_BASE)
		return 0;
	
	if(reduced_ratio < 5000)
	{
		printk("%s:reduced_ratio(%d) out of range", __func__, reduced_ratio);
		reduced_ratio = 5000;
	}

	tar_psctrl_reg = (atomic_read(&obj->psctrl_val) & 0x0F) + 1;
	reduced_ratio <<= 1;
	temp_para = (uint32_t) 64 * REDUCE_RATIO_BASE / reduced_ratio;
	tar_ledctrl_reg = (uint8_t) temp_para + 1;
	if(tar_ledctrl_reg > MAX_LED_DUTY)
		tar_ledctrl_reg = MAX_LED_DUTY;
	tar_psctrl_reg = (atomic_read(&obj->psctrl_val)  & 0xF0) | tar_psctrl_reg;
	
	if((ret = stk3x1x_write_ps(obj->client, tar_psctrl_reg)))
	{
		APS_ERR("write ps error: %d\n", ret);
		return ret;        
	}
	tar_ledctrl_reg = (obj->ledctrl_val & 0xC0) | tar_ledctrl_reg;

	if((ret = stk3x1x_write_led(obj->client, tar_ledctrl_reg)))
	{
		APS_ERR("write ps error: %d\n", ret);
		return ret;        
	}
	printk("%s: reg=0x%x,0x%x\n", __func__, tar_psctrl_reg, tar_ledctrl_reg);
	return 0;
}
/*----------------------------------------------------------------------------*/
#define ESTIMATE_DARK40(dark25)		(dark25 * 34 / 10)

static int32_t stk_ps_tune2_compen(struct stk3x1x_priv *obj)
{
	int32_t ps_comp = REDUCE_RATIO_BASE;
	int16_t est_temp = 0;	
	static int8_t debounce = 0;
	int32_t dark_code = 0;
	int32_t psoff_code = 0;
	int32_t ret;
	...
	ret = stk3x1x_get_dark_reading(obj->client);
	if(ret < 0)
		return ret;
	dark_code = ret;
	stk_ps_tune2_normalize_dark(atomic_read(&obj->alsctrl_val), &dark_code);
	ret = stk3x1x_get_psoff_reading(obj);
	if(ret < 0)
		return ret;
	psoff_code = ret;
	stk_ps_tune2_normalize_psoff(atomic_read(&obj->psctrl_val), &psoff_code);
	printk("%s:PS:%5d,%5d,%5d,%5d\n", 
		__func__, obj->ps, obj->als_code_last, dark_code, psoff_code);
		
	if(psoff_code > STK_PS0OFF_TOO_LARGE_THD)
	{
		printk(KERN_ERR "%s: psoff(%d) is too large\n", __func__, psoff_code);
		return -EINVAL;
	}

	dark_code -= ((uint32_t)psoff_code * 441 / 10000);
	if(dark_code > (ESTIMATE_DARK40(obj->dark25) - 5))
	{
		...
		ps_comp = 1416 * (dark_code - ESTIMATE_DARK40(obj->dark25)) + 400000;
		est_temp = ps_comp / 10000;
		if(est_temp < 40)
		{
			// printk("%s: est_temp(%d) out of range", __func__, est_temp);
			return 0;
		}
		if(est_temp > 60)
		{
			est_temp = 60;
			ps_comp = est_temp * 10000;			
		}
		
		ps_comp = 12000 - 5 * ps_comp / 1000;			
		// ps_comp = 13716 - 123 * ps_comp / 10000;	// 10000*ps_ratio
		stk_ps_tune2_adjust_duty_it(obj, (uint16_t)ps_comp);
		printk("%s:COMP:%3d,%6d\n", __func__, est_temp, ps_comp);			
	}
	else if(dark_code < (ESTIMATE_DARK40(obj->dark25)  - 10))
	{	
		if(debounce <= 0 && debounce > -100)
			debounce--;
		else
		...
		if(debounce > -3)
			...
			stk_ps_tune2_adjust_def_duty_it(obj);
	}
...
/*----------------------------------------------------------------------------*/
#if defined(CONFIG_HAS_EARLYSUSPEND)
static int32_t stk_enable_tune2_suspend(struct stk3x1x_priv *obj, uint8_t start)
{
	int ret;
	uint8_t alsctrl;
	...
	if(start)
	{
		stk3x1x_write_als_high_thd(obj->client, 0x0);
		stk3x1x_write_als_low_thd(obj->client, 0xFFFF);
		alsctrl = atomic_read(&stk3x1x_obj->alsctrl_val) | 0xC0;
		if((ret = stk3x1x_write_als(obj->client, alsctrl)))
			return ret;        			

	if(0 == obj->hw->polling_mode_als)
	{
		if((ret = stk3x1x_write_int(obj->client, obj->int_val | 0x08)))
			return ret;        
	}
		if((ret = stk3x1x_write_state(obj->client, 0x3)))
			return ret;
		
		obj->tune2_suspended = true;		
	}
	else
	{
		if((ret = stk3x1x_write_als(obj->client, atomic_read(&stk3x1x_obj->alsctrl_val))))
			return ret;  
	if(0 == obj->hw->polling_mode_als)
	{
		if((ret = stk3x1x_write_int(obj->client, obj->int_val)))
			return ret;   
	}
		if((ret = stk3x1x_write_state(obj->client, 0x5)))
			return ret;
		obj->tune2_suspended = false;
	}
	return 0;
}
#endif
#endif	// #ifdef STK_TUNE2
/*----------------------------------------------------------------------------*/
#ifdef STK_TUNE0	
...
#ifdef STK_TUNE1_WITH_ALS_COMPEN
static int stk_tune1_judge_als_nf(struct stk3x1x_priv *obj, int *nf_flag)
{
	if(*nf_flag == 1 || obj->als_code_last >= STK_ALS_FAR)
	{
		*nf_flag = 1;
		return 1;
	}
	else if(*nf_flag == 0 && obj->als_code_last <= STK_ALS_NEAR)
	{
		*nf_flag = 0;
		return 2;		
	}
	*nf_flag = obj->ps_distance_last;
	return 0;
}
#endif
...
#ifdef STK_TUNE1_WITH_ALS_COMPEN
static int stk_tune1_als_int_handle(struct stk3x1x_priv *obj, u16 als_reading)
{
	u16 ps_reading = 0;
	int err;
	
	if(obj->ps_distance_last == 0 && als_reading >= STK_ALS_FAR)
	{
		if((err = stk3x1x_read_ps(obj->client, &ps_reading)))
		{
			APS_ERR("stk3x1x read ps data: %d\n", err);
			return err;
		}
		printk("%s:handle_state=%d, als=%d,ps=%d, als far, force ps interrupt\n", __func__, 
			obj->stk_int_handle_state, als_reading, ps_reading);
		atomic_set(&obj->ps_high_thd_val, ps_reading + obj->stk_ht_n_ct); 
		atomic_set(&obj->ps_high_thd_val, ps_reading + obj->stk_lt_n_ct);
		obj->ps = ps_reading;
...
static int stk_tune1_ps_int_handle(struct stk3x1x_priv *obj)
{
...
#ifdef STK_TUNE1_WITH_ALS_COMPEN
	if((ret = stk3x1x_read_als(obj->client, &als_reading)))
	{
	...
	obj->als = als_reading;
	stk_tune1_judge_als_nf(obj, &nf_state);
#endif
...
#ifdef STK_TUNE1_WITH_ALS_COMPEN
			stk3x1x_write_als_high_thd(obj->client, STK_ALS_FAR);
			stk3x1x_write_als_low_thd(obj->client, 0);
			printk("%s:(N) HT=%d,LT=%d,ALS_HT=%d,ALS_LT=0\n", __func__, STK_FIN_THD, ps_thd_l, STK_ALS_FAR);				
#else
	...
	case STK_PSINT_CT_DET:
		...
		if(!nf_state)
		{
			...
			stk3x1x_write_als_high_thd(obj->client, STK_ALS_FAR);
			stk3x1x_write_als_low_thd(obj->client, 0);
			printk("%s:(C) HT=%d,LT=%d,ALS_HT=%d,ALS_LT=0\n", __func__, STK_FIN_THD, ps_thd_l, STK_ALS_FAR);				
#else
...
static int stk_als_int_handle(struct stk3x1x_priv *obj, u16 als_reading)
{
	...
	//let up layer to know
	obj->als_code_last = als_reading;
#if (defined(STK_TUNE2) || defined(STK_TUNE1_WITH_ALS_COMPEN))	
	if(obj->als_en_hal == false)
		return 0;
#endif	
...
#ifdef STK_TUNE1_WITH_ALS_COMPEN
		err = stk_tune1_als_int_handle(obj, obj->als);
		if(err < 0)
			goto err_i2c_rw;
#endif
...
static int stk3x1x_init_client(struct i2c_client *client)
{
	...
	*/
	...
	obj->als_en_hal = false;
#ifdef STK_TUNE1	
	obj->stk_int_handle_state = STK_PSINT_NORM;
#endif
#ifdef STK_TUNE2		
	err = stk3x1x_read_otp26(client);
	if(err < 0)
		return err;
	...
	if(err != 0)
		...
		obj->dark25 = STK_FT_AVE_DARK_25DEG;
...
static int stk3x1x_get_ps_value(struct stk3x1x_priv *obj, u16 ps)
{
	...
	// obj->ps_code_last = reading;
#ifdef STK_TUNE2
	stk_ps_tune2_compen(obj);
#endif	
#ifdef STK_TUNE1
	#ifdef STK_TUNE1_WITH_ALS_COMPEN
		stk_tune1_judge_als_nf(obj, &val);
	#endif		
...
{   /*early_suspend is only applied for ALS*/
	...
	if(old & STK_STATE_EN_ALS_MASK)
	{
	#if (defined(STK_TUNE2))
		if(old & STK_STATE_EN_PS_MASK)
			stk_enable_tune2_suspend(obj, 1);
	#elif defined(STK_TUNE1_WITH_ALS_COMPEN)
		if(old & STK_STATE_EN_PS_MASK)
		{
			printk("%s: keep ALS enabled\n", __func__);	
		}
	#else
...
#ifdef STK_TUNE2
	if(old & STK_STATE_EN_PS_MASK)
		stk_enable_tune2_suspend(obj, 0);	
#elif (!defined(STK_TUNE1_WITH_ALS_COMPEN))
...
#if (defined(STK_TUNE1_WITH_ALS_COMPEN) || defined(STK_TUNE2))
				if(obj->als_en_hal == 0)
				{
					err = stk3x1x_enable_als(obj->client, value);
					if(err){
						APS_ERR("enable als is failed!!\n");
						return -1;
					}	
				}
#endif	// #if (defined(STK_TUNE1_WITH_ALS_COMPEN) || defined(STK_TUNE2))
			}
			break;
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				value = *(int *)buff_in;				
#ifdef STK_TUNE1_WITH_ALS_COMPEN
				if(atomic_read(&stk3x1x_obj->state_val) & STK_STATE_EN_PS_MASK)
				{
					printk("%s: keep ALS enabled\n", __func__);
				}
				else
				...
				obj->als_en_hal = value?true:false;
			}
			break;
...
static int als_enable_nodata(int en)
{
	...
	stk3x1x_obj->als_en_hal = en?true:false;	
	return 0;
}
...
//	obj->hw = get_cust_alsps_hw();
	...
	atomic_set(&obj->alsctrl_val, 0x3B);
...
//	ps_ctl.is_report_input_direct = false;
	...
	if(1 == obj->hw->polling_mode_ps)
	{
		ps_ctl.is_polling_mode = 1; //true;
		ps_ctl.is_report_input_direct = false;
	}
	else
	{
		ps_ctl.is_polling_mode = 0; //false;
...
{
	const char *name = "mediatek,arg_stk3x1x"; //"mediatek,stk3x1x";
