...
#define REGFLAG_DELAY 0xFE
     ...
     LCM_DRIVER *g_lcm_drv = NULL;//add by xiaopu.zhu for hardwareinfo
  #endif
int _lcm_count(void)
{
	return lcm_count;
...
//Begin add by xiaobin.zhai for lcd device tree
	sprintf(dts_params, "mediatek,lcm_params-%s", lcm_drv->name);
//End add by xiaobin.zhai for lcd device tree
...
//Begin add by xiaobin.zhai for lcd device tree
	sprintf(dts_ops, "mediatek,lcm_ops-%s", lcm_drv->name);
//ENd add by xiaobin.zhai for lcd device tree	
...
//End add by xiaobin.zhai for lcd device tree
		...
		if (plcm_name == NULL) {
			...
			for (i = 0; i < _lcm_count(); i++) {
				...
				if (!strcmp(lcm_drv->name, plcm_name)) {
					...
					isLCMInited = true;
					lcmindex = i;
					DISPERR("found lcm driver:%s   %s in linux kernel driver\n",lcm_drv->name,plcm_name);
					break;
				}
	...
	if (isLCMFound == false) {
       ...
       #if defined (CONFIG_HW_INFO) 
       g_lcm_drv = lcm_drv;//add by xiaopu.zhu for hardware info
	#endif
