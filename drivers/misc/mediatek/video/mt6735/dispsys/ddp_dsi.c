...
#else
#include "disp_dts_gpio.h"	//add by llf for pinctl
#endif
#include "ddp_mmp.h"
#include <mt-plat/mt_gpio.h>
...
long lcd_vci_bias_setting(unsigned int value)
{
	long ret = 0;
#if !defined(CONFIG_MTK_LEGACY)
	if (value)
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BIAS_VCI1);
	else
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BIAS_VCI0);
#endif
	return ret;
}

long lcd_vdd_bias_setting(unsigned int value)
{
	long ret = 0;
#if !defined(CONFIG_MTK_LEGACY)
	if (value)
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BIAS_VDD1);
	else
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BIAS_VDD0);
#endif
	return ret;
}
...
#ifdef CONFIG_SHINE_4G
	static int esd_flg = -1;
#endif
	DSI_T0_INS t0;
...
#ifdef CONFIG_SHINE_4G
#define GPIO_LCM_ERR_FLG 	(GPIO103 | 0x80000000)   //GPIO103_ERR_FLG
		mt_set_gpio_mode(GPIO_LCM_ERR_FLG, 0);
		mt_set_gpio_dir(GPIO_LCM_ERR_FLG, GPIO_DIR_IN);
		mt_set_gpio_pull_enable(GPIO_LCM_ERR_FLG, GPIO_PULL_DISABLE);

		esd_flg=mt_get_gpio_in(GPIO_LCM_ERR_FLG);
		printk("[DSI_LLF] ESD check flag=%d\n", esd_flg);
#endif
		...
		for (i = 0; i < 3; i++) {
			...
			/* / 0xB0 is used to distinguish DCS cmd or Gerneric cmd, is that Right??? */
			t0.Data_ID = (t0.Data0 < 0xFF) ? DSI_DCS_READ_PACKET_ID :
			    			DSI_GERNERIC_READ_LONG_PACKET_ID;
			t0.Data1 = 0;
...
#ifdef CONFIG_SHINE_4G
			if(esd_flg==0){
				//printk("[DSI_CMP] ESD check flag=%d\n", esd_flg);
				ret=0; /* esd pass */
			}else{
				//printk("[DSI_CMP] ESD check flag=%d\n", esd_flg);
				ret=1; /* esd fail */
