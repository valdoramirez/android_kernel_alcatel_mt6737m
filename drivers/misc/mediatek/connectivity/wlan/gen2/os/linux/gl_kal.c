...
kalIndicateStatusAndComplete(IN P_GLUE_INFO_T prGlueInfo, IN WLAN_STATUS eStatus, IN PVOID pvBuf, IN UINT_32 u4BufLen)
{
	...
	//Added by jiabin.zheng.hz for Defect4410707 on 2017/04/10 begin
	OS_SYSTIME rCurrentTime;
	BOOLEAN fgIsNeedUpdateBss = FALSE;
	//Added by jiabin.zheng.hz for Defect4410707 on 2017/04/10 end
	...
	case WLAN_STATUS_MEDIA_CONNECT:
		...
		if (prGlueInfo->fgIsRegistered == TRUE) {
			...
			//Modified by jiabin.zheng.hz for Defect4410707 on 2017/04/10 begin
			//if (bss == NULL) {
			...
			if ((bss == NULL) || (TRUE == fgIsNeedUpdateBss)) {
				/* create BSS on-the-fly */
			//Modified by jiabin.zheng.hz for Defect4410707 on 2017/04/10 end
				if (prBssDesc != NULL) {
