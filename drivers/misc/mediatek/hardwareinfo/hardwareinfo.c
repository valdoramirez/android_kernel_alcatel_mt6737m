...
char g_main_camera_otp = 0;
char g_sub_camera_otp = 0;
/****************************************************************************** 
 ...
 * Debug configuration
******************************************************************************/
//add by liuxin for eMMC information begin
static u8 *emmc_spt_tab[][4] = {
	/*:::*/
	{"150100464A32354142", "KMFJ20005A_B213",		"Samsung",		"4G+512M"},
	{"150100464A32374D42", "KMFJ20007M_B214",		"Samsung",	"4G+768M"},
	{"150100464E58324D42", "KMFNX0012M_B214",		"Samsung",		"8G+1G"},
	{"90014A483847346132", "H9TQ64A8GTCCUR_KUM",		"Hynix",	"8G+1G"},
};
//add by liuxin for eMMC information begin
...
//begin 20160310 liujunting modify for tp0 issue
		...
		ret_value += sprintf(buf+ret_value, "FW_version:0x%x:", tpd->tp_firmware_version);
...
static ssize_t show_eMMC_id(struct device *dev,struct device_attribute *attr, char *buf)
{
...
static ssize_t store_eMMC_id(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    printk("[%s]: Not Support Write Function\n",__func__);  
    return size;
}
...
static ssize_t show_main_camera_otp(struct device *dev,struct device_attribute *attr, char *buf)
{
    int ret_value = 1;

        ret_value = sprintf(buf , "main camera otp :%d\n", g_main_camera_otp);
  
    return ret_value;
}
static ssize_t store__main_camera_otp(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    printk("[%s]: Not Support Write Function\n",__func__);  
    return size;
}
...
static ssize_t show_sub_camera_otp(struct device *dev,struct device_attribute *attr, char *buf)
{
    int ret_value = 1;
    ret_value = sprintf(buf , "sub camera otp :%d\n", g_sub_camera_otp);
    
    return ret_value;
}
static ssize_t store_sub_camera_otp(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    printk("[%s]: Not Support Write Function\n",__func__);  
    return size;
}
...
static DEVICE_ATTR(main_camera_otp, 0644, show_main_camera_otp, store__main_camera_otp);
static DEVICE_ATTR(sub_camera_otp, 0644, show_sub_camera_otp, store_sub_camera_otp);
...
static DEVICE_ATTR(eMMC, 0644, show_eMMC_id, store_eMMC_id);
...
{   
    ...
    if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_main_camera_otp)) != 0) goto exit_error;
    if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_sub_camera_otp)) != 0) goto exit_error;
    ...
    if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_eMMC)) != 0) goto exit_error;
exit_error: 
...
static int HardwareInfo_driver_remove(struct platform_device *dev)
{
 ...
 //   device_remove_file(&(dev->dev), &dev_attr_ctp);
    ...
    device_remove_file(&(dev->dev), &dev_attr_main_camera_otp);
    device_remove_file(&(dev->dev), &dev_attr_sub_camera_otp);    
	...
	device_remove_file(&(dev->dev), &dev_attr_eMMC);
    return 0;
}
