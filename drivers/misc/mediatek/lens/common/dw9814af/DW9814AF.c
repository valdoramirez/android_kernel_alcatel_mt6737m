...
static int s4AF_WriteReg(u16 a_u2Data)
{
	...
	if (i4RetValue < 0) {
		LOG_INF("I2C send failed!!addr:0x%x,error number:%d  data: %d,%d\n",g_pstAF_I2Cclient->addr,i4RetValue,puSendCmd[0],puSendCmd[1]);
		return -1;
	}
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		if(i4RetValue<0)
		LOG_INF("I2C send failed!!addr:0x%x,error number:%d  data: %d,%d\n",g_pstAF_I2Cclient->addr,i4RetValue,puSendCmd2[0],puSendCmd2[1]);
		else
		LOG_INF("I2C send success!addr:0x%x",g_pstAF_I2Cclient->addr);
		i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd3, 2);
		if(i4RetValue<0)
		LOG_INF("I2C send failed!!addr:0x%x,error number:%d  data: %d,%d\n",g_pstAF_I2Cclient->addr,i4RetValue,puSendCmd3[0],puSendCmd3[1]);
		else
		LOG_INF("I2C send success!addr:0x%x",g_pstAF_I2Cclient->addr);
		i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd4, 2);
		if(i4RetValue<0)
		LOG_INF("I2C send failed!!addr:0x%x,error number:%d  data: %d,%d\n",g_pstAF_I2Cclient->addr,i4RetValue,puSendCmd4[0],puSendCmd4[1]);
		else
		LOG_INF("I2C send success!addr:0x%x",g_pstAF_I2Cclient->addr);
		i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd5, 2);
		if(i4RetValue<0)
		LOG_INF("I2C send failed!!addr:0x%x,error number:%d  data: %d,%d\n",g_pstAF_I2Cclient->addr,i4RetValue,puSendCmd5[0],puSendCmd5[1]);
		else
		LOG_INF("I2C send success!addr:0x%x",g_pstAF_I2Cclient->addr);
