...
/* ///////////////////////////////////////////////////////////////////////////////////////// */
/* // add by qiuguang.liang for task for task 3380494////Create File For EM : Charging_Current */
/* ///////////////////////////////////////////////////////////////////////////////////////// */
static ssize_t show_Charging_Current(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	battery_log(BAT_LOG_CRTI, "[EM] show_Charging_Current : %d\n", battery_meter_get_charging_current());
	return sprintf(buf, "%d\n", battery_meter_get_charging_current());
}
...
/*[326405] battery dicharged too fast by liming.zhu.hz at 20150614 bigen*/
//BEGIN-xin_liu add for MINI Software shutdown charging requirement Task3697043 2016-12-12
    ...
    if (BMT_status.bat_charging_state == CHR_HOLD
        ...
        bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_NOT_CHARGING;
		printk("[BATTERY] chr hold: soc=%d\n", BMT_status.SOC);
    }
#endif
//END-xin_liu add for MINI Software shutdown charging requirement Task3697043 2016-12-12
...
void mt_battery_GetBatteryData(void)
{
	...
	/*--Begin-20160406-jrdhz-linyaosen-modify-for mmitest charge dectect--*/
    #ifdef TARGET_BUILD_MMITEST
    static signed int bat_sum, temperature_sum;
    #else  
	static signed int bat_sum, icharging_sum, temperature_sum;
	static signed int batteryCurrentBuffer[BATTERY_AVERAGE_SIZE];
	#endif
	/*--End-20160406-jrdhz-linyaosen-modify-for mmitest charge dectect--*/
...
#define CHG_MAX_VALID_BATT_ID_BYD  1500
#define CHG_MIN_VALID_BATT_ID_VEKEN  0
...
{	
	...
	/* begin: modify by binpeng.huang on 2016.03.31 for Defect 1862690 */
    if ((BMT_status.id_vol > CHG_MIN_VALID_BATT_ID_VEKEN) && (BMT_status.id_vol < CHG_MAX_VALID_BATT_ID_BYD))
	{
	}
	else if ((BMT_status.id_vol > CHG_MIN_VALID_BATT_ID_SCUD) && (BMT_status.id_vol < CHG_MAX_VALID_BATT_ID_SCUD))
	{
	}
	else
    {
		...
		#ifdef CONFIG_MTK_KERNEL_POWER_OFF_CHARGING //Modified by qiuguang.liang
	...
	/* end: modify by binpeng.huang on 2016.03.31 for Defect 1862690 */
...
//begin modify by qiuguang.liang for defect 3562885
#ifndef TARGET_BUILD_MMITEST
		...
		if (BMT_status.temperature >= 60 ||BMT_status.temperature <= -20) {
...
#else
			{
				if ((g_platform_boot_mode == META_BOOT)
					...
					battery_log(BAT_LOG_CRTI,
						    "[Battery] Tbat is %d, system need power down.\n",
						    BMT_status.temperature);
...
//end modify by qiuguang.liang for defect 3562885

static void mt_battery_update_status(void)
{
//begin-add by qiuguang.liang
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Charging_Current);//add by qiuguang.liang for task 3380494
