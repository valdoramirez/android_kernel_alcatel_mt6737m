...
//willab start  
/*if user is using some file, for example, playing a video. Then the file is locked at that time.
So the file must be forbidden to read*/
#ifdef CONFIG_LOCK_FOR_READ
    if (!get_lockhide_flag()) {
        if ((file->f_dentry->d_inode->i_flags & S_LOCK) == S_LOCK)
        {
            printk(KERN_ERR "  hello vfs_read is lock,  name is %s\n", file->f_dentry->d_name.name);
            return -EACCES;
        }
    }
#endif	
